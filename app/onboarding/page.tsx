...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    async function loadSession() {
      try {
        ...
        if (!response.ok) throw new Error(data.error || "Unable to load session details.");
        if (cancelled) return;
        ...
        if (!cancelled) {
          setFeedback(error instanceof Error ? error.message : "Unable to load session details.");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await response.json()) as { success?: boolean; error?: string; portalUrl?: string };
      ...
      setFeedback(error instanceof Error ? error.message : "Unable to complete onboarding.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="mx-auto max-w-6xl px-4 py-28 sm:px-6 lg:px-8">
      <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr]">
        <div className="rounded-[32px] border border-white/10 bg-gradient-to-b from-brand/10 to-white/[0.03] p-8 shadow-soft">
          ...
          <h1 className="mt-6 text-4xl font-black tracking-tight">{headerTitle}</h1>
          ...
          <div className="mt-8 space-y-3 text-sm text-white/85">
            <p>• {isSpanish ? "Se enlaza con la suscripción real que ya pagaste" : "It links to the real subscription you already paid for"}</p>
            <p>• {isSpanish ? "Marca onboarding como completo" : "Marks onboarding as complete"}</p>
            <p>• {isSpanish ? "Envía resumen al correo operativo si está configurado" : "Sends a summary to the ops inbox if configured"}</p>
          </div>

          <div className="mt-8 rounded-[24px] border border-white/10 bg-black/20 p-5 text-sm text-white/85">
            <div className="font-semibold text-white">{session?.planTitle || "Dexcore"}</div>
            <div className="mt-2 text-muted">{session?.customerEmail || form.email || "—"}</div>
        ...
        <div className="rounded-[32px] border border-white/10 bg-white/[0.04] p-8 shadow-soft">
          <form className="grid gap-4" onSubmit={handleSubmit}>
            <div className="grid gap-4 md:grid-cols-2">
              <Input label={isSpanish ? "Nombre de contacto" : "Contact name"} value={form.contactName} onChange={(value) => setForm((current) => ({ ...current, contactName: value }))} required />
              <Input label={isSpanish ? "Empresa" : "Company"} value={form.companyName} onChange={(value) => setForm((current) => ({ ...current, companyName: value }))} required />
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <Input label="Email" type="email" value={form.email} onChange={(value) => setForm((current) => ({ ...current, email: value }))} required />
              <Input label={isSpanish ? "Teléfono" : "Phone"} value={form.phone} onChange={(value) => setForm((current) => ({ ...current, phone: value }))} />
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <Input label="Website" value={form.website} onChange={(value) => setForm((current) => ({ ...current, website: value }))} placeholder="https://" />
              <Input label={isSpanish ? "Servicio principal" : "Main service"} value={form.serviceType} onChange={(value) => setForm((current) => ({ ...current, serviceType: value }))} required />
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <Input label={isSpanish ? "Área de servicio" : "Service area"} value={form.serviceArea} onChange={(value) => setForm((current) => ({ ...current, serviceArea: value }))} required />
              <Input label={isSpanish ? "Fecha objetivo de lanzamiento" : "Target launch date"} type="date" value={form.preferredLaunchDate} onChange={(value) => setForm((current) => ({ ...current, preferredLaunchDate: value }))} />
            </div>

            <Textarea label={isSpanish ? "Meta principal" : "Primary goal"} value={form.goals} onChange={(value) => setForm((current) => ({ ...current, goals: value }))} required placeholder={isSpanish ? "Ejemplo: quiero más citas agendadas y responder más rápido." : "Example: I want more booked calls and faster lead response."} />
            <Textarea label={isSpanish ? "Notas adicionales" : "Additional notes"} value={form.notes} onChange={(value) => setForm((current) => ({ ...current, notes: value }))} placeholder={isSpanish ? "Accesos, restricciones, links, horarios o cualquier detalle importante." : "Access, restrictions, links, hours, or any detail that matters."} />
            ...
            >
              {submitting ? (isSpanish ? "Guardando..." : "Saving...") : isSpanish ? "Completar onboarding" : "Complete onboarding"}
              <ArrowRight size={16} />
            </button>
          </form>
          ...
          {feedback && (
            <div className="mt-6 rounded-[22px] border border-white/10 bg-black/20 p-4 text-sm text-white/90">
              ...
              {portalUrl && (
                <Link href={portalUrl} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-brand">
                  {isSpanish ? "Abrir portal del cliente" : "Open client portal"}
                  <ArrowRight size={16} />
                </Link>
              )}
            </div>
